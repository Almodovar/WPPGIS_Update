...
func HandleModelRun(w http.ResponseWriter, r *http.Request) {
	...
	dbSpatial, err := sql.Open("sqlite3", "./static/administration/assets/swat/spatial.db3")
	checkErr(err)
	...
	f, err := os.Create("./static/administration/assets/swat/webs.webs")
	checkErr(err)
	...
	err = ioutil.WriteFile("./static/administration/assets/swat/webs2.webs", []byte(websString), 0644)
	if err != nil {
		panic(err)
	}
	...
	go func() {
		...
		cmd.Dir = "C:/xxKun_Learn/Go/Go/src/WPPGIS_Update./static/administration/assets/swat"
		cmd.Run()
		done <- true
...
func HandleEcoOutletChart(w http.ResponseWriter, r *http.Request) {
	var d string
	err := json.NewDecoder(r.Body).Decode(&d)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
...
func HandleEcoCostEffectiveness(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	if d.FeatureType == "field" {
		...
		if costChange != 0 {
			costeffectiveness[0] = (compareTofieldAverage[d.FeatureID].Water - compareFromfieldAverage[d.FeatureID].Water) * 1000 / costChange
			costeffectiveness[1] = (compareTofieldAverage[d.FeatureID].Sediment - compareFromfieldAverage[d.FeatureID].Sediment) * 1000 / costChange
			costeffectiveness[2] = (compareTofieldAverage[d.FeatureID].Tn - compareFromfieldAverage[d.FeatureID].Tn) * 1000 / costChange
			costeffectiveness[3] = (compareTofieldAverage[d.FeatureID].Tp - compareFromfieldAverage[d.FeatureID].Tp) * 1000 / costChange
	...
	if d.FeatureType == "subbasin" {
		...
		if costChange != 0 {
			costeffectiveness[0] = (compareTosubbasinAverage[d.FeatureID].Water - compareFromsubbasinAverage[d.FeatureID].Water) * 1000 / costChange
			costeffectiveness[1] = (compareTosubbasinAverage[d.FeatureID].Sediment - compareFromsubbasinAverage[d.FeatureID].Sediment) * 1000 / costChange
			costeffectiveness[2] = (compareTosubbasinAverage[d.FeatureID].Tn - compareFromsubbasinAverage[d.FeatureID].Tn) * 1000 / costChange
			costeffectiveness[3] = (compareTosubbasinAverage[d.FeatureID].Tp - compareFromsubbasinAverage[d.FeatureID].Tp) * 1000 / costChange
			costeffectiveness[4] = float64(d.FeatureID)
...
func BasintoField(database string) {
	...
	db, err := sql.Open("sqlite3", "./static/administration/assets/swat/"+database+".db3")
	checkErr(err)
...
func OutletResultArray(s string) {
	...
	db, err := sql.Open("sqlite3", "./static/administration/assets/swat/"+s+".db3")
	checkErr(err)
...
func OutletCompareResultArray(s string) {
	...
	db, err = sql.Open("sqlite3", "./static/administration/assets/swat/result.db3")
	checkErr(err)
...
func HandleOptimizationLimites(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	check(err)
	err = ioutil.WriteFile("./static/administration/assets/WEBsOptimization/optimization.txt", optimizationConfigJson, 0644)
	check(err)
	...
	go func() {
		...
		cmd := "java"
		args := []string{"-jar", "./static/administration/assets/WEBsOptimization/WEBsInterface_WB.jar"}
	...
	if _, err := os.Stat("./static/administration/assets/WEBsOptimization/done.txt"); err == nil {
		fmt.Println("complete")
	}

	fileString := readFile("./static/administration/assets/WEBsOptimization/UpLow.txt")
...
func HandleOptimizationRun(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	optResultDB := createSQLConnector("./static/administration/assets/WEBsOptimization/OptOut.db3")
...
func updateEconomicBySubbasinID(subbasinID int, bmpCode int) map[int]*EcoResult {
	...
	spatialDB, err := sql.Open("sqlite3", "./static/administration/assets/swat/spatial.db3")
	checkErr(err)
